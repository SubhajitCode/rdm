...
async fn test_http_downloader_end_to_end_with_range_server() {
    ...
    // Mount the range-aware responder for all GET requests
    ...
    assert_eq!(
        output.len(),
        body_size,
        "assembled file size should equal original body size"
    );
    assert_eq!(
        output, body,
        "assembled file content should match original byte-for-byte"
    );
...
async fn test_http_downloader_non_resumable() {
    let body_size = 64 * 1024; // 64 KB
    let body = generate_test_data(body_size);

    let server = MockServer::start().await;

    // Always return 200 (non-resumable)
    Mock::given(method("GET"))
        .respond_with(
            ResponseTemplate::new(200)
...
async fn test_http_downloader_stop_during_download() {
    ...
    // Slow catch-all for download piece requests — respond slowly
    Mock::given(method("GET"))
        .respond_with(
    ...
    // Give the download a moment to start spawning tasks
    tokio::time::sleep(std::time::Duration::from_millis(200)).await;

    // Stop the download
    strategy.stop().await.unwrap();

    // The download should complete within a reasonable time.
    ...
    // After stop, the download may succeed or fail — both are acceptable.
    assert!(
...
async fn test_http_downloader_progress_events_received() {
    ...
    // Collect progress events in a background task
    ...
    downloader.download().await.unwrap();
    // Drop downloader so the channel sender is dropped, ending the receiver
    drop(downloader);

    let (total, count) = progress_handle.await.unwrap();
    assert_eq!(
        total, body_size as u64,
        "total progress bytes should equal body size"
    );
